...
function rowReturn(type, client) {
	...
	case 1: // 9 Grid
		const target1 = client.extra.random(0, 9);
		const output1 = [];
		for(let j = 0; j < 3; j++) {
			const rows1 = new MessageActionRow();
			for(let i = 0; i < 3; i++) {
				if((j * 3) + i == target1) rows1.addComponents(new MessageButton().setCustomId('hit').setLabel('Hit! ').setStyle('DANGER'));
				else rows1.addComponents(new MessageButton().setCustomId('miss' + (j * 3) + i).setLabel('Miss!').setStyle('SECONDARY'));
			}
			output1.push(rows1);
		}
			...
			if(phase == 0) {
				...
				if (i.customId === 'hit' || i.customId === 'superhit') {
					...
					if(!cast_yes.includes(i.user.id) && !cast_no.includes(i.user.id)) {
						...
						try{ await i.reply({ content: 'You already tried to dodge!', ephemeral: true }); }
						catch {client.extra.log_error_g(client.logger, message.guild, 'Boss Event', 'Miss Attacik Denied');}
					}
					...
					if(!cast_yes.includes(i.user.id) && !cast_no.includes(i.user.id)) {
						...
						try{ await i.reply({ content: 'You already tried to interrupt the cast!', ephemeral: true }); }
						catch {client.extra.log_error_g(client.logger, message.guild, 'Boss Event', 'Miss Attacik Denied');}
					}
				...
				if(damage < health && collector.ended == false) {
					if(second >= sec_limit && cast < 1) {
						...
						combatEmbed = new MessageEmbed()
							...
							.setDescription('The boss is charging at you! **Dodge it!**')
