...
function rowReturn(type, client, disable) {
	...
	case 0: // Punch n deflect
		const target0 = client.extra.random(0, 9);
		const output0 = [];
		for(let j = 0; j < 3; j++) {
			const rows0 = new MessageActionRow();
			for(let i = 0; i < 3; i++) {
				if((j * 3) + i == target0) rows0.addComponents(new MessageButton().setCustomId('deflect').setLabel('Deflect!').setStyle('PRIMARY'));
	...
	case 1: // 10 Grid Battle
		const target1 = client.extra.random(0, 10);
		const output1 = [];
		for(let j = 0; j < 2; j++) {
			const rows1 = new MessageActionRow();
			for(let i = 0; i < 5; i++) {
				if((j * 5) + i == target1) rows1.addComponents(new MessageButton().setCustomId('hit').setLabel('Hit! ').setStyle('DANGER').setDisabled(disable));
				else rows1.addComponents(new MessageButton().setCustomId('miss' + (j * 5) + i).setLabel('Miss!').setStyle('SECONDARY').setDisabled(disable));
			}
			output1.push(rows1);
		}
		return output1;
	...
	case 2: // Finish
		const target2 = client.extra.random(0, 25);
		const output2 = [];
		for(let j = 0; j < 5; j++) {
			const rows2 = new MessageActionRow();
			for(let i = 0; i < 5; i++) {
				if((j * 5) + i == target2) rows2.addComponents(new MessageButton().setCustomId('hit').setLabel('Hit! ').setStyle('DANGER'));
				else rows2.addComponents(new MessageButton().setCustomId('miss' + (j * 5) + i).setLabel('Miss!').setStyle('SECONDARY'));
			}
			output2.push(rows2);
		}
		return output2;
...
module.exports = {
	async execute(message, client, interactionMessage) {
		...
		let combatEmbed = new MessageEmbed()
			...
			.setDescription('The boss is charging up for a magic storm!')
			.setFooter('Get ready!');
			...
			if(phase == 0) {
				...
				if (i.customId.startsWith('hit') || i.customId === 'superhit') {
					...
					if (stunned.includes(i.user.id)) {
						...
						try{ await i.reply({ content: 'You already tried deflecting!', ephemeral: true }); }
						catch {client.extra.log_error_g(client.logger, message.guild, 'Boss Event', 'Miss Attacik Denied');}
					}
		...
		while (damage < health && collector.ended == false) {
			if(phase == 1 && (100 - Math.floor((damage / health) * 100)) < 60) {
				...
				embed = new MessageEmbed()
					...
					// eslint-disable-next-line spaced-comment
					//.setThumbnail(user.defaultAvatarURL)
					.setDescription('**The boss isn\'t giving up just yet!**')
					.setFooter('This is the end!')
					.setImage(asset.image_url);
				...
				sec_limit = client.extra.random(4, 6);
				combatEmbed = new MessageEmbed()
					.setColor(client.colors[1])
					.setTitle('Action Belt')
					// eslint-disable-next-line spaced-comment
				...
				if(damage < health && collector.ended == false) {
					if(second >= sec_limit) {
						...
						let string = '';
						sec_limit = 1;
						if(cast_yes.length > 0) string = cast_name.join(', ') + ' deflected the attack and the boss took some damage!\n\n';
						...
						combatEmbed = new MessageEmbed()
							...
							.setDescription(string + 'Magic rans from the sky! **Deflect** it to the boss!')
				...
				if(damage < health && collector.ended == false) {
					if(second >= sec_limit && cast < 1) {
						...
						combatEmbed = new MessageEmbed()
							...
							.setDescription('The boss vanished! You can\'t attack it until it appears again!')
						...
						combatEmbed = new MessageEmbed()
							...
							.setDescription('The boss appeared again! Give it all you got!')
							.setFooter('Next Button Swap is in ' + sec_limit + ' seconds.');
						cast = 1;
						if(damage < health && collector.ended == false) {
							try{await interactionMessage.edit({ embeds: [embed, combatEmbed], components: rows }).then(client.extra.log_g(client.logger, message.guild, 'Message Create Event', 'Boss Attack Edit'));}
