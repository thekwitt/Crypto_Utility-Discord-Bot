...
module.exports = {
	name: 'lookup',
	alias: ['lu'],
	guide: '!lookup [Contract or Slug/Collection Name] [Token ID] or !lu [Contract or Slug/Collection Name] [Token ID (or Random)]\n\nEx: !lookup skullx 1234 or !lu 0x1dc5d3b2162f9500d7ddec14eb0ba9ccb43bc20c 1234',
	async execute(message, client, m) {
		const strings = message.content.toLowerCase().split(' ');
		if(strings[1] == undefined || strings[2] == undefined) {
		...
		let embed = new MessageEmbed()
			...
			.setDescription('⠀\nThank you for your order! Your nft is being summoned through ' + (client.extra.random(0, 101) > 95 ? 'brokensea' : 'opensea') + '!\n⠀')
		...
		if(strings[2].toLowerCase() === 'random') {
			stats = await client.extra.getJsonOS('https://api.opensea.io/api/v1/collection/' + strings[1] + '/stats');
		} else if(isNaN(strings[2]) == true) {
		...
		let id = undefined;
		if(stats != undefined) id = client.extra.random(0, stats.stats.count);
		else id = strings[2];
		...
		try {
			if(stats == undefined) assets = await client.extra.getBasicAssets(client, reform, strings[1]);
			else assets = await client.extra.getJsonOS('https://api.opensea.io/api/v1/assets?include_orders=true&order_direction=desc&offset=' + reform + '&limit=1&collection=' + strings[1]);
		} catch {
			embed = new MessageEmbed()
				.setTitle('🔴   Summon Canceled!   🔴')
		...
		embed = new MessageEmbed()
			.setTitle((asset.name != undefined ? asset.name : '#' + asset.token_id))
		...
		for(let j = 0; j < traits.length; j++) {
			const type = traits[j].trait_type;
			embed.addField(type, traits[j].value + ' (' + ((traits[j].trait_count / 10000) * 100).toFixed(2) + '%)', true);
		}
		...
		try {
			data = await client.extra.getBasicBundle(client, reform, strings[1]);
		} catch {;}
